...
def create_button_counter_table(event, context):
    try:
        ...
        return { "statusCode": 400, "body": "Failed to create button_counter table!", "message": e }
    finally:
        if (connection is not None and connection.open):
            connection.close()
    # response = { "statusCode": 201, "body": "Successfuly created button_counter table!" }
    response = { "statusCode": 201, "body": str(password) }
    return response
...
def delete_button_counter_table(event, context):
    try:
        ...
        response = { "statusCode": 400, "body": "Failed to delete button_counter table!", "message": e }
    finally:
        if (connection is not None and connection.open):
            connection.close()
...
def insert_click_button_counter_table(event,context):
    try:
        ...
        response = { "statusCode": 400, "body": "Failed to insert row into button_counter table!", "message": e }
    finally:
        if (connection is not None and connection.open):
            connection.close()
